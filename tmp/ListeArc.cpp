...
{
    /*if (find(listeA.begin(), listeA.end(), a) != listeA.end())
    {
        map <Arc>::iterator it = find(listeA.begin(), listeA.end(), a);
        int index = distance(listeA.begin(), it);
        listeA[index].Incremente();
    }
    else
    {
        listeA.push_back(a);
